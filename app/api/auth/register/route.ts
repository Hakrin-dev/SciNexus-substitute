...
export async function POST(req: NextRequest) {
  ensureSeed();
  if (!allowRequest(req, "auth-register", 5, 60 * 60_000)) return fail("注册次数过多，请稍后再试", 429, "RATE_LIMITED");
  try {
    ...
    if (body.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email.trim())) return fail("邮箱格式不正确", 422, "INVALID_EMAIL");
