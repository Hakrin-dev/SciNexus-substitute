...
export async function DELETE(req:NextRequest,{params}:{params:Promise<{id:string;userId:string}>}) {
  ensureSeed(); const actor=requireAuth(req); if(!actor)return fail("请先登录",401,"UNAUTHORIZED"); const {id,userId}=await params;
  ...
  if(!member)return fail("成员不存在",404); if(member.role==="owner")return fail("不能移除项目所有者",422);
  getDB().prepare("DELETE FROM project_members WHERE project_id=? AND user_id=?").run(id,userId);
  writeAudit({userId:actor.id,projectId:id,action:"project.member.remove",resourceType:"project_member",resourceId:userId}); return ok({deleted:true});
}
