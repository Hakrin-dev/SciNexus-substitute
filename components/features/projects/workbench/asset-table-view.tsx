...
import { ArrowUpRight, ChevronDown, Database, FolderKanban, FolderOpen, Plus, Search, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { useCreateWorkbenchAsset } from "@/lib/api/services";
...
export function AssetTableView({ projectId, projectName, assets, selection, onSelect, readOnly = false, questionId }: Props) {
  const router = useRouter();
  const createAsset = useCreateWorkbenchAsset(projectId);
  const [query, setQuery] = useState("");
  const [kind, setKind] = useState<AssetKind | "all">("all");
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState("");
  const [newKind, setNewKind] = useState<AssetKind>("paper");
  const [newStage, setNewStage] = useState<(typeof STAGE_FOLDERS)[number][0]>("read");
  ...
  const folders = useMemo(() => STAGE_FOLDERS.map(([key, label]) => ({
    key, label, items: filtered.filter((asset) => {
      ...
      const stage = String(asset.artifact?.stage || asset.tags.find((tag) => STAGE_FOLDERS.some(([candidate]) => candidate === tag)) || inferred);
      return stage === key;
    }),
    ...
    <section className="rounded-2xl bg-card p-6 shadow-card">
      <header className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5">
          {!readOnly && <button onClick={() => setShowCreate((value) => !value)} className="flex h-9 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-medium text-white"><Plus className="size-3.5" />新增资产</button>}
      ...
      {showCreate && !readOnly && (
        <form
          className="mt-4 grid gap-3 rounded-2xl border border-primary/20 bg-primary-soft/20 p-4 md:grid-cols-2"
          onSubmit={(event) => {
            event.preventDefault();
            const title = newTitle.trim();
            if (!title) return;
            const tags = [...new Set(newTags.split(/[，,]/).map((tag) => tag.trim()).filter(Boolean))];
            createAsset.mutate(
              {
                kind: newKind,
                title,
                meta: newMeta.trim(),
                status: newKind === "paper" ? "unread" : "active",
                tags: [...tags, newStage],
        ...
        >
          <label className="space-y-1"><span className="text-[11px] text-muted">资产标题 *</span><input value={newTitle} onChange={(event) => setNewTitle(event.target.value)} placeholder="论文、数据集、实验或笔记名称" className="h-9 w-full rounded-lg border border-line bg-card px-3 text-xs text-ink outline-none focus:border-primary" /></label>
          <label className="space-y-1"><span className="text-[11px] text-muted">说明</span><input value={newMeta} onChange={(event) => setNewMeta(event.target.value)} placeholder="来源、格式、版本或实验状态" className="h-9 w-full rounded-lg border border-line bg-card px-3 text-xs text-ink outline-none focus:border-primary" /></label>
          <label className="space-y-1"><span className="text-[11px] text-muted">类型</span><select value={newKind} onChange={(event) => setNewKind(event.target.value as AssetKind)} className="h-9 w-full rounded-lg border border-line bg-card px-3 text-xs text-ink outline-none">{KIND_FILTERS.filter((item) => item.value !== "all").map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
          <label className="space-y-1"><span className="text-[11px] text-muted">研究阶段</span><select value={newStage} onChange={(event) => setNewStage(event.target.value as (typeof STAGE_FOLDERS)[number][0])} className="h-9 w-full rounded-lg border border-line bg-card px-3 text-xs text-ink outline-none">{STAGE_FOLDERS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="space-y-1 md:col-span-2"><span className="text-[11px] text-muted">标签</span><input value={newTags} onChange={(event) => setNewTags(event.target.value)} placeholder="多个标签用逗号分隔" className="h-9 w-full rounded-lg border border-line bg-card px-3 text-xs text-ink outline-none focus:border-primary" /></label>
          <div className="flex justify-end gap-2 md:col-span-2"><button type="button" onClick={() => setShowCreate(false)} className="flex h-8 items-center gap-1 rounded-lg px-3 text-xs text-muted"><X className="size-3.5" />取消</button><button disabled={!newTitle.trim() || createAsset.isPending} type="submit" className="h-8 rounded-lg bg-primary px-4 text-xs font-medium text-white disabled:opacity-50">{createAsset.isPending ? "保存中…" : "保存资产"}</button></div>
        </form>
      )}
