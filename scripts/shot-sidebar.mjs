...
// ① 首页:侧边栏应为 研究/库/历史 三分区,「发现」默认展开学者/机构
await waitText("课题工作台");
await sleep(600);
await shot("g_side_1_home.png");

// ② 点「工具库」主标题 → 展开 技能/插件/MCP(禁用态)
console.log(
  "click 工具库:",
  await evalJs(`(() => {
    const els = [...document.querySelectorAll("button")];
    const el = els.find((x) => x.innerText.trim() === "工具库" && x.offsetParent !== null);
    if (!el) return false;
    el.click();
    return true;
  })()`),
...
await sleep(400);
await shot("g_side_2_tools.png");

// ③ 点「知识库」主标题 → 跳 /knowledge 并展开 论文/笔记/记忆
console.log(
  "click 知识库:",
  await evalJs(`(() => {
    const els = [...document.querySelectorAll("button")];
    const el = els.find((x) => x.innerText.trim() === "知识库" && x.offsetParent !== null);
    if (!el) return false;
    el.click();
    return true;
  })()`),
...
await sleep(800);
await shot("g_side_3_knowledge.png");
